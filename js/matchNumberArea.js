var MatchNumber = require('./matchNumber');
...
var MatchNumberArea = {
    ...
    init: function (game,parent,offX,offY,revealGroup,showingGroup){
        ...
        //this.gameBoardBlue = parent.create(offX,offY,'BB_boardBlue');
        //this.gameBoardBlue.anchor.set(0.5);
        ...
        for(var row=0;row<1;row++){
            for(var i=0;i<7;i++){
                var x1= i*this.matchNumberWidth;
                var y1 = row*this.matchNumberHeight;
                var smallIcon = this.bagHolder.create(x1,y1-1,'smallTile');
                //smallIcon.scale.set(0.69);
                var thisNumberCell1 = Object.create(MatchNumber);
                thisNumberCell1.init(this.game,x1,y1,this.numbersHolder,this.matchNumberWidth,this.matchNumberHeight,this.offX,this.offY, this.revealFinalHolder);
                this.allNumbers.push(thisNumberCell1);
            }
        }

        this.BMDWidth = this.matchNumberWidth*7;
        this.BMDHeight = this.matchNumberHeight;

        this.revealBMDAll = game.make.bitmapData(640, 960);
        ...
        //this.revealBMD.circle(0, 0, 2000);
        //this.revealFinalImage = this.revealFinalHolder.addChild(game.add.image(0, 0, this.revealBMD));
    ...
    setBoard: function (numbers, matchedNumbers) {
        ...
        this.allNumbers.forEach(function(element){
            element.setNumber(this.revealBMD, numbers[tempN]);
            if(matchedNumbers.indexOf(numbers[tempN])>-1) element.markMatch();
    ...
    autoReveal: function () {
        ...
        //this.autoRevealTimerEvent = this.game.time.events.loop(400, this.autoRevealSingle, this);
        this.revealAllCell();
    },
    ...
    autoRevealSinglePath: function (obj) {
        this.autoRevealPointer.x = obj.cellX+30;
        this.autoRevealPointer.y = obj.cellY+38;
        this.isAutoRevealing = true;

        var tween1 = this.game.add.tween(this.autoRevealPointer).to({x:'+50',y:'-0'},80,"Linear",true);
        var tween2 = this.game.add.tween(this.autoRevealPointer).to({x:'-0',y:'30'},80,"Linear");
        var tween3 = this.game.add.tween(this.autoRevealPointer).to({x:'-65',y:'-0'},60,"Linear");
    ...
    paintAll: function () {
        ...
        //    //this.revealMaskBMD.update();
        //    //this.revealFinalBMD.alphaMask(this.revealBMD, this.revealMaskBMD);
        //    //this.revealFinalImage.alpha -= 0.1;
        //    //console.log("fade");
        //}, this);
...
module.exports = MatchNumberArea;
